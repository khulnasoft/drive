...
type Analysis struct {
	...
	Source                    drive.ImageSource `yaml:"-" mapstructure:"-"`
...
func DefaultAnalysis() Analysis {
	return Analysis{
		...
		IgnoreErrors:              false,
		AvailableContainerEngines: drive.ImageSources,
	}
}
...
func (c *Analysis) PostLoad() error {
	...
	if c.Image != "" {
		sourceType, imageStr := drive.DeriveImageSource(c.Image)

		if sourceType == drive.SourceUnknown {
			sourceType = drive.ParseImageSource(c.ContainerEngine)
			if sourceType == drive.SourceUnknown {
		...
		c.Source = drive.ParseImageSource(c.ContainerEngine)
	}
