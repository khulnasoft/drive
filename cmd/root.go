...
func initCli() {
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.drive.yaml, ~/.config/drive/*.yaml, or $XDG_CONFIG_HOME/drive.yaml)")
	rootCmd.PersistentFlags().String("source", "docker", "The container engine to fetch the image from. Allowed values: "+strings.Join(drive.ImageSources, ", "))
	...
	rootCmd.Flags().StringVar(&ciConfigFile, "ci-config", ".drive-ci", "If CI=true in the environment, use the given yaml to drive validation rules.")
...
func initConfig() {
	...
	viper.SetDefault("log.path", "./drive.log")
...
// and in $HOME/.config/drive/ directory
// defaults to $HOME/.drive.yaml
func getDefaultCfgFile() string {
	...
	return path.Join(home, ".drive.yaml")
}

// findInPath returns first "*.yaml" file in path's subdirectory "drive"
// if not found returns empty string
func findInPath(pathTo string) string {
	directory := path.Join(pathTo, "drive")
