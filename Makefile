...
bootstrap-tools: $(TEMP_DIR)
	...
	curl -sSfL https://raw.githubusercontent.com/khulnasoft/go-bouncer/master/bouncer.sh | sh -s -- -b $(TEMP_DIR)/ $(GOBOUNCER_VERSION)
...
#run: build
#	$(BUILD_PATH) build -t drive-example:latest -f .data/Dockerfile.example .
#
#run-large: build
#	$(BUILD_PATH) amir20/clashleaders:latest
#
#run-podman: build
#	podman build -t drive-example:latest -f .data/Dockerfile.example .
#	$(BUILD_PATH) localhost/drive-example:latest --engine podman
#
#run-podman-large: build
#	$(BUILD_PATH) docker.io/amir20/clashleaders:latest --engine podman
#
#run-ci: build
#	CI=true $(BUILD_PATH) drive-example:latest --ci-config .data/.drive-ci
#
#dev:
...
generate-test-data:
	docker build -t drive-test:latest -f .data/Dockerfile.test-image . && docker image save -o .data/test-docker-image.tar drive-test:latest && echo 'Exported test data!'
...
ci-test-docker-image:
	docker run \
		...
		'${PRODUCTION_REGISTRY}/khulnasoft/drive:latest' \
			'${TEST_IMAGE}' \
			--ci
	...
	docker run \
		...
		ubuntu:latest \
			/bin/bash -x -c "\
				...
				docker version && \
				apt install ./snapshot/drive_*_linux_amd64.deb -y && \
				drive --version && \
				drive '${TEST_IMAGE}' --ci \
			"
		...
		fedora:latest \
			/bin/bash -x -c "\
				curl -L 'https://download.docker.com/linux/static/stable/x86_64/docker-${DOCKER_CLI_VERSION}.tgz' | \
					...
					mv docker /usr/local/bin/ &&\
				docker version && \
				dnf install ./snapshot/drive_*_linux_amd64.rpm -y && \
				drive --version && \
				drive '${TEST_IMAGE}' --ci \
			"
...
ci-test-linux-run:
	ls -la $(SNAPSHOT_DIR)
	ls -la $(SNAPSHOT_DIR)/drive_linux_amd64_v1
	chmod 755 $(SNAPSHOT_DIR)/drive_linux_amd64_v1/drive && \
	$(SNAPSHOT_DIR)/drive_linux_amd64_v1/drive '${TEST_IMAGE}'  --ci && \
    $(SNAPSHOT_DIR)/drive_linux_amd64_v1/drive --source docker-archive .data/test-kaniko-image.tar  --ci --ci-config .data/.drive-ci
...
ci-test-mac-run:
	chmod 755 $(SNAPSHOT_DIR)/drive_darwin_amd64_v1/drive && \
	$(SNAPSHOT_DIR)/drive_darwin_amd64_v1/drive --source docker-archive .data/test-docker-image.tar  --ci --ci-config .data/.drive-ci

# we're not attempting to test docker, just our ability to run on these systems. This avoids setting up docker in CI.
.PHONY: ci-test-windows-run
ci-test-windows-run:
	drive.exe --source docker-archive .data/test-docker-image.tar  --ci --ci-config .data/.drive-ci
